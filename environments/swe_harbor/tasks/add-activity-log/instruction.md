...
- **Log only on success.** Do not create an activity record when the view returns 400 (validation error), 403 (forbidden), 404 (not found), or 409 (e.g. "check is not paused" on resume). Only log immediately before returning a successful JSON response (201 or 200).
- **Details length.** If you pass `details` (e.g. check name) into the logger, truncate to 500 characters before saving so the `CharField(max_length=500)` constraint is never violated.
- **Project scoping.** The list endpoint must return only activity for the project identified by the API key. No cross-project data must ever appear. Deleting a project must delete its activity logs (use `CASCADE` on the ForeignKey).
- **Response shape.** Each activity item in the list must have exactly the keys `action`, `check_code`, `details`, and `created`. The `created` value must be ISO 8601 with no microseconds (e.g. `2025-02-17T12:00:00+00:00`), and `check_code` must be a string UUID or `null`.
